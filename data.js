...
export async function stationSeries(source, station, where, cols, limit) {
  limit = limit || STATION_DETAIL_MAX;
  const sid = source.stations.indexOf(station);
  if (sid < 0) return { n: 0, t: new Float64Array(0), match: new Uint8Array(0), cols: [] };
  ...
  const flag = where ? `CAST((${where}) AS BOOLEAN)` : "FALSE";
  const rel = projected(source, `"${SID_COL}" = ${sid}`);
  const sel = ['CAST(epoch_ms("DATE_TIME_PARSED") AS DOUBLE) AS t_ms']
    .concat(want.map((c) => `CAST("${c}" AS DOUBLE) AS "${c}"`))
    .concat([`${flag} AS "${MATCH_COL}"`]);
  const sql =
    `SELECT ${sel.join(", ")} FROM ${rel} ORDER BY "DATE_TIME_PARSED" ` +
    `LIMIT ${Math.floor(limit)}`;
  const res = await qCached(sql);
  const out = { n: res.n, t: num(res, "t_ms"), match: new Uint8Array(res.n) };
  const m = res.cols[MATCH_COL] || [];
  for (let i = 0; i < res.n; i++) out.match[i] = m[i] ? 1 : 0;
